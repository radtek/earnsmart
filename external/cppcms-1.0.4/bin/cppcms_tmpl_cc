...
class inputbox(base_show):
    #pattern=r'<%cpptextbox[\s]+((name=[\"][\w|\W]+[\"])\s*(class=[\"][\w|\W]+[\"])?\s*(placeholder=[\"][\w|\W]+[\"])?\s*(validate=[\"].+[\"])?\s*(value=[\"][\w|\W]+[\"]))\s*[^%>]*%>'
    pattern=r'<%cpptextbox\s+(([\w\W]+)=[\'|\"]([\w\W]+)[\'|\"]\s+)[^%>]*%>'
    basic_pattern = "cpptextbox"
    basic_name = 'cpptextbox'
    attribpattern = r'([a-zA-Z]+=[\'|\"][a-zA-Z0-9\s\^\$\(\)\*@\]\[\{\}_\-\.,]*[\'|\"])'

    def use(self, m):
        output("out() << " + "\"<input \";")
        attribs = m.group(1)
        f = re.compile(self.attribpattern, re.DOTALL)
        m = f.findall(attribs)
        isrequired = False
        validate = False
        fieldName = None
        ...
        for attr in m :
            ...
            if attrn not in ["value", "required", "validate"]:
                output("out() << " + " \" " + to_string(attr) + "\"" + ";")
                if attrn == "name":
                    fieldName = attrv
            elif attrn == "value":
                fieldValue = attrv.replace("'","").replace('"',"")
                output('out() << " value=\\"\" << ' + self.prepare(fieldValue) + ' << "\\"";')
            elif attrn == "required":
                isrequired = True
            elif attrn == "validate":
                validate=True
        output("out() << \" >\"" + ";")
        #if the content.app().request().request_method()=="POST" then show the validation error
        #if required is set also show the required character.
        if isrequired :
            lbl = '\"<label style=\\"color:red\\">\" << ' + self.prepare('RequiredStr') + ' << "</label>\";'
            output("out() << " + lbl)
        if validate :
            functor = "content.get_Validated(" + fieldName + ", " + make_ident(fieldValue) + ")"
            output('if (content.app().request().request_method()=="POST") {')
            output('\tstd::string validationtext=%s;' % functor)
            output('\tif (!validationtext.empty()){')
            lbl = '\"<label style=\\"color:red\\">\" << validationtext << "</label>\";'
            output("\t\tout() << " + lbl)
            output('\t}')
            output('}')
