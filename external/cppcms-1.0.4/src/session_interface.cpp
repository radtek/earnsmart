...
void session_interface::set_callback(ICallback* callback)
{
	pCallback = callback;
}

bool session_interface::is_cbset()
{
	return pCallback != 0;
}
...
bool session_interface::load()
{
	...
	if(!storage_->load(*this,ar,timeout_in_)) 
        {
        	if (pCallback) pCallback->OnLoad(*this);	
		return false;
	}
	load_data(data_,ar);
	data_copy_=data_;
...
void session_interface::save()
{
	...
	if (pCallback) pCallback->OnSave(*this);	
...
int session_interface::browsersession() 
{
    return context_->service().cached_settings().session.browsersession;
}
