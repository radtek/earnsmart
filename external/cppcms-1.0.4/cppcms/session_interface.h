...
struct ICallback
{
  	virtual void OnLoad(session_interface& session) = 0;
	virtual void OnSave(session_interface& session) = 0;
};
...
public:
	...
	///
	/// Set callback to know when the session is loaded and saved. Set to null to unset the callback
	///
	void set_callback(ICallback* callback = 0);
	...
	enum {
        ...
        ///
        /// Get the 'browsersession' settings from configuration file.
        int browsersession();
