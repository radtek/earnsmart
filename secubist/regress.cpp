...
namespace se
{
    namespace math
    {
        ...
        void CCubist::Regress(CaseNo Fp, CaseNo Lp, double *Model)
        /*   -------  */
        {
            CaseNo	i, Kp;
            int		j, jj, k, kk;
            ...
            GEnv.Mean[Att] /= Cases;
            GEnv.Var[Att]   = (GEnv.Var[Att] - Cases * GEnv.Mean[Att]
                      * GEnv.Mean[Att]) / (Cases-1);
            ...
            FindModelAtts(Model);
            if ( ! GEnv.NModelAtt ) return;	/* just a mean */
        ...
        void CCubist::BuildTables(CaseNo Fp, CaseNo Lp)
        /*   -----------  */
        {
            int		i, j, jj, k, kk;
            ContValue	ClassVal, JVal;
        ...
        void CCubist::Solve(double *Model)
        /*   -----  */
        {
            int		j, k, jj, max;
            ...
            if ( Pivot < MinPivot )
            {
                ...
                Verbosity(3,
                printf("=== no pivot for %s (%g, max elt=%g)\n",
            ...
            ForEach(j, 0, GEnv.NModelAtt)
            {
            Model[GEnv.ModelAtt[j]] = GEnv.B[j];
            }
        }
        ...
        void CCubist::FindActiveAtts(void)
        /*   --------------  */
        {
            Attribute	Att;
            ...
            {
            if ( ! GEnv.ZeroCoeff[Att] ) GEnv.ModelAtt[GEnv.NModelAtt++] = Att;
            }
            GEnv.NModelAtt--;
        }
        ...
        void CCubist::AddRow(double *Model, short From, short To, double Factor)
        /*   ------  */
        {
            short	col;
            double	*ATo, *AFrom;
        ...
        void CCubist::ExchangeRow(double *Model, short From, short To)
        /*   -----------  */
        {
            short	col;
            double	HoldVal;
        ...
        int CCubist::CountCoeffs(double *Model)
        /*  -----------  */
        {
            int		Number=1;
            Attribute	Att;
        ...
        void CCubist::SimplifyModel(DataRec *D, CaseNo Fp, CaseNo Lp, double *Model)
        /*   -------------  */
        {
            double	ModErr, AdjModErr, BestAdjErr=1E10, Contrib, LeastContrib,
                Wt, Cases=0;
            CaseNo	i;
            Attribute	Att, Drop;
            int		a;
            ...
            ForEach(a, 1, GEnv.NModelAtt)
            {
                ...
                if ( Contrib > 1000 * GEnv.AvDev[0] ) Stable = false;
            }
            ...
            if ( Stable && Cases >= 2 * GEnv.NModelAtt )
            {
                ...
                Verbosity(3,
                fprintf(Of, "Cases %d:%d  mod err=%.2f/%.2f%s\n",
                    Fp, Lp, ModErr, AdjModErr,
            ...
            {
                Verbosity(3,
                fprintf(Of, "Drop %s  coeff=%.3f  mean=%.3f  |dev|=%.3f\n",
