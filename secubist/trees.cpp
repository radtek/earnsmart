...
namespace se
{
    namespace math
    {
        ...
        void CCubist::PrintTree(Tree T, __String Title)
        /*   ---------  */
        {
            int s;
        ...
        void CCubist::Show(Tree T, int Sh)
        /*   ---- */
        {
        ...
        void CCubist::ShowBranch(int Sh, Tree T, DiscrValue v, DiscrValue BrNo)
        /*   ----------  */
        {
            DiscrValue	Pv, Last;
            Attribute	Att;
            ...
            case BrSubset:
                ...
                if ( In(Pv, T->Subset[v]) )
                {
                    Extra = ( Pv != Last || T->Branch[v]->NodeType ? 0 :
                           6 );
                    if ( ! FirstValue &&
        ...
        void CCubist::Indent(int Sh, int BrNo)
        /*   ------  */
        {
            int	i;
        ...
        /*   --------  */
        {
            ...
            FreeUnlessNil(T->Model);					T->Model = Nil;
            FreeUnlessNil(T->MCopy);					T->MCopy = Nil;
            Free(T);
        }
        ...
        Tree CCubist::Leaf(CaseCount Cases, double Mean, double SD)
        /*   ----  */
        {
            Tree	Node;
        ...
        void CCubist::Sprout(Tree T, DiscrValue Branches)
        /*   ------  */
        {
            T->Forks = Branches;
        ...
        int CCubist::TreeLeaves(Tree T)
        /*  ----------  */
        {
            int		Sum=0;
            DiscrValue	v;

            if ( ! T || T->Cases < 1 ) return 0;

            if ( T->NodeType )
            {
            ForEach(v, 1, T->Forks)
            {
                Sum += TreeLeaves(T->Branch[v]);
            }
